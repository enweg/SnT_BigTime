...
<p style="color:red">
Below are the results of bootUR. We can see that some series are non-stationary while others are. Differencing only some series might be weird because the interpretation will be rather difficult. Differencing all series might also be weird because quite some are I(0) and we might hence over-difference. Solution might be to work with cointegration but not sure how this would work in a penalised setting. How would you like to continue? You used the data before, how did you handle the non-stationarity? For now I will not do anything.
</p>

```{r}
# oi <- bootUR::order_integration(df.rv.raw, plot_orders = TRUE)
