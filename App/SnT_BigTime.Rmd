...
We can estimate a model by calling `sparseVAR(Y, selection = "cv")` where `Y` is the matrix containing our data (each variable/index is one column) and `selection="cv"` tells the function that we would like to do time series cross-validation to select the penalty parameter. 
...
* Although this introduces a bias into the estimation, the Bias-Variance trade-off tells us that if the increase in bias is small compared to the decrease in variance, high-dimensional methods can outperform traditional methods in the MSE sense
* *glmnet* cannot be used for the estimation of sparse VAR models because
    1. The sparsity pattern corresponding to the lasso does not account for the dynamic nature of the VAR model. A  hierachical sparsity structure is needed, such as the one enforced by the *elementwise* sparsity structure
